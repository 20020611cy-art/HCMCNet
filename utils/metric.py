...
def compute_score(hist, correct, labeled):
    ...
    f1 = 2 * precision_1 * recall_1 / (precision_1 + recall_1 + 1e-8)
    ...
    pe = ((hist.sum(axis=0) * hist.sum(axis=1)).sum()) / (total * total)
    kappa = (po - pe) / (1 - pe + 1e-8)



    return iou, recall_1, precision_1, mean_IoU, mean_IoU_no_back, freq_IoU, mean_pixel_acc, pixel_acc,f1, kappa
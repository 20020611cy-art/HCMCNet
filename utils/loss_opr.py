...
class CELossDiceLoss(nn.Module):
    def __init__(self, alpha=0.5, smooth=1e-8):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        # Dice loss (二分类示例，适合变化检测)
        probs = F.softmax(logits, dim=1)
        # 只针对前景类
        probs_fg = probs[:, 1, :, :]
        targets_fg = (targets == 1).float()
        intersection = (probs_fg * targets_fg).sum()
        dice_loss = 1 - (2 * intersection + self.smooth) / (probs_fg.sum() + targets_fg.sum() + self.smooth)
...
def lovasz_hinge(logits, labels):
    ...
    losses = []
    for b in range(B):
        loss = lovasz_hinge_flat(logits[b].view(-1), labels[b].view(-1))
        losses.append(loss)
    return sum(losses) / B
...
# CE + Lovasz 组合损失 (二分类)
# -----------------------------
class BinaryCELovaszLoss(nn.Module):
    def __init__(self, alpha=0.7, ignore_index=255):
        """
        alpha: CE权重, Lovasz权重 = 1 - alpha
        """
        super(BinaryCELovaszLoss, self).__init__()
        self.alpha = alpha
        self.ce_loss = nn.CrossEntropyLoss(ignore_index=ignore_index)

    def forward(self, logits, labels):
        """
        logits: [B, 2, H, W] 未softmax
        labels: [B, H, W]
        """
        # CE损失
        ...
        # Lovasz损失
        lovasz = lovasz_hinge(logits[:,1,:,:], labels.float())

        loss = self.alpha * ce + (1 - self.alpha) * lovasz
        return loss